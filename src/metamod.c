...
int mod_metadata(int argc, char *argv[], int optind, metadata_op operation,
...
int main(int argc, char *argv[]) {
    ...
    switch (meta_op) {
        case META_ADD:
            ...
            int status = mod_metadata(argc, argv, optind, meta_op,
...
int mod_metadata(int argc, char *argv[], int optind, metadata_op operation,
    ...
    while (optind < argc) {
        ...
        if (status < 0) {
            error_count++;
            logmsg(ERROR, METAMOD_CAT, "Failed to resolve path '%s'", path);
        }
        else {
    ...
    logmsg(DEBUG, METAMOD_CAT, "Processed %d paths with %d errors",
...
error:
    ...
    logmsg(ERROR, METAMOD_CAT, "Processed %d paths with %d errors",
           path_count, error_count);
